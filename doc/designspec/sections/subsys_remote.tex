...
\item[stopplinje] \mono{stop} om markerad som destination, annars
    \mono{ignore}.
\item[parkeringsficka (ej dest.)] \mono{park} om markerad som destination,
    annars \mono{ignore}.
\item[rondell] En \mono{enter/exit}, därefter en \mono{ignore} för varje avfart
    den ska passera och slutligen en \mono{enter/exit}.
\end{labeling}
