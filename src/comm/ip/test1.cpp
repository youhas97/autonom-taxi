...
void plotLane(cv::Mat& original_frame, std::vector<cv::Point>& points) {
	...
	/*std::string message = "De rånar dig!!!";
	...
	double dist;
	dist = std::sqrt(abs((exp2(points[1].x - img_center_pt)) + exp2(points[1].y - 720)));
...
int main() {
	...
	while (true) {
		...
		double image_center_pt = static_cast<double>((frame.cols / 2));
